...
def perturb(data_source, epoch, corpus_test, hybrid=False):
    ...
    with open("./output/%s/%s_perturbation.txt" % (args.outf, epoch), "a") as f:
        for batch in data_source:
            ...
            ce1 = c + torch.FloatTensor(c.size()).uniform_(-0.001, 0.001)
            ce2 = c + torch.FloatTensor(c.size()).uniform_(-0.01, 0.01)
            ce3 = c + torch.FloatTensor(c.size()).uniform_(-0.1, 0.1)

            re1 = gan_gen(z + torch.FloatTensor(z.size()).uniform_(-0.001, 0.001))
            re2 = gan_gen(z + torch.FloatTensor(z.size()).uniform_(-0.01, 0.01))
            re3 = gan_gen(z + torch.FloatTensor(z.size()).uniform_(-0.1, 0.1))

            batch_size = premise.size(0)
            for i in range(batch_size):
                f.write("========================================================\n")
                tt = hypothesise_words[i][1:]
                if "<pad>" in tt:
                    tt = tt[:tt.index("<pad>")]
                f.write(" ".join(tt) + "\n")

                roc_c_idx = autoencoder.generate(c[i], 10, True).data.cpu().numpy()[0]
                word_c = [corpus_test.dictionary.idx2word[x] for x in roc_c_idx]
                if "<eos>" in word_c:
                    word_c = word_c[:word_c.index("<eos>")]
                f.write(" ".join(word_c)+ "\n")

                roc_r_idx = autoencoder.generate(r[i], 10, True).data.cpu().numpy()[0]
                word_r = [corpus_test.dictionary.idx2word[x] for x in roc_r_idx]
                if "<eos>" in word_r:
                    word_r = word_r[:word_r.index("<eos>")]
                f.write(" ".join(word_r)+ "\n")

                f.write("--------------------------------------------------------\n")
                roc_c_idx = autoencoder.generate(ce1[i], 10, True).data.cpu().numpy()[0]
                word_c = [corpus_test.dictionary.idx2word[x] for x in roc_c_idx]
                if "<eos>" in word_c:
                    word_c = word_c[:word_c.index("<eos>")]
                f.write(" ".join(word_c)+ "\n")

                roc_c_idx = autoencoder.generate(ce2[i], 10, True).data.cpu().numpy()[0]
                word_c = [corpus_test.dictionary.idx2word[x] for x in roc_c_idx]
                if "<eos>" in word_c:
                    word_c = word_c[:word_c.index("<eos>")]
                f.write(" ".join(word_c)+ "\n")

                roc_c_idx = autoencoder.generate(ce3[i], 10, True).data.cpu().numpy()[0]
                word_c = [corpus_test.dictionary.idx2word[x] for x in roc_c_idx]
                if "<eos>" in word_c:
                    word_c = word_c[:word_c.index("<eos>")]
                f.write(" ".join(word_c)+ "\n")

                roc_r_idx = autoencoder.generate(re1[i], 10, True).data.cpu().numpy()[0]
                word_r = [corpus_test.dictionary.idx2word[x] for x in roc_r_idx]
                if "<eos>" in word_r:
                    word_r = word_r[:word_r.index("<eos>")]
                f.write(" ".join(word_r)+ "\n")

                roc_r_idx = autoencoder.generate(re2[i], 10, True).data.cpu().numpy()[0]
                word_r = [corpus_test.dictionary.idx2word[x] for x in roc_r_idx]
                if "<eos>" in word_r:
                    word_r = word_r[:word_r.index("<eos>")]
                f.write(" ".join(word_r)+ "\n")

                roc_r_idx = autoencoder.generate(re3[i], 10, True).data.cpu().numpy()[0]
                word_r = [corpus_test.dictionary.idx2word[x] for x in roc_r_idx]
                if "<eos>" in word_r:
                    word_r = word_r[:word_r.index("<eos>")]
                f.write(" ".join(word_r)+ "\n")           
